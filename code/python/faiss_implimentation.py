...
def main():
    ...
    sdf = df.sample(10, random_state=123)  # Sample 10 documents for searching
    srch_txt_list = sdf.text.tolist()
    
    # Process search documents
    print('\n***Search: Use case specific processing:')
    print('\n Truncating document(s):')
    srch_txt_list = classifier.doc_processor.keep_lines(srch_txt_list, num_lines=4)
    
    # Perform chunking on search documents
    print('\n***Search: Perform chunking:')
    print(f'CHUNK_SIZE:{CHUNK_SIZE} OVERLAP:{OVERLAP}')
    srch_lolo_chunks = classifier.doc_processor.get_chunks(srch_txt_list)
    ...
    # Perform search and probability extraction
    print('\n***Search in vectordb:')
    k = 2  # Number of results per query
    
    # Flatten chunks for searching
    flat_chunks = [chunk for chunks in srch_lolo_chunks for chunk in chunks]
    
    # V1: Probability extraction using basic similarity
    results_v1 = classifier.batch_search(flat_chunks, k=k)
    
    # Process and print results
    print('\n***Results Summary:')
    print(f'Total search docs: {len(results_v1)}')
    print(f'Results per query: {k}')
    
    # Calculate and store probabilities
    probabilities = []
    for doc_idx, doc_results in enumerate(results_v1):
        print(f'\n*** Results for document {doc_idx}:')
        ...
        for result in doc_results:
            ...
            for i, (dist, meta, prob) in enumerate(zip(distances, metadatas, probs)):
                similarity = 1 - dist if classifier.vectordb.similarity_algo == 'l2' else dist
                print(f'\nMatch {i+1}:')
    ...
    # V2: Enhanced probability extraction with metadata
    print('\n***V2 Probability Extraction:')
    label_probabilities = {}
    
    for doc_results in results_v1:
        doc_labels = []
        doc_similarities = []
        
        for result in doc_results:
            for dist, meta in zip(result['distances'], result['metadatas']):
                similarity = 1 - dist if classifier.vectordb.similarity_algo == 'l2' else dist
                doc_labels.append(meta['label'])
                doc_similarities.append(similarity)
        
        # Calculate probabilities per label
        unique_labels = list(set(doc_labels))
        label_sims = {label: [] for label in unique_labels}
        
        for label, sim in zip(doc_labels, doc_similarities):
            label_sims[label].append(sim)
        
        # Average similarities per label
        label_probs = {label: np.mean(sims) for label, sims in label_sims.items()}
        
        # Normalize probabilities
        total = sum(label_probs.values())
        label_probs = {label: prob/total for label, prob in label_probs.items()}
        
        # Store results
        for label, prob in label_probs.items():
            if label not in label_probabilities:
                label_probabilities[label] = []
            label_probabilities[label].append(prob)
    
    # Print final probability distribution per label
    print('\nFinal Label Probability Distribution:')
